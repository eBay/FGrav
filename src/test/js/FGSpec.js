...
describe("FG", function() {
    ...
    describe("when loading", function() {
       ...
       it("should generate DynamicallyLoading objects from names", function () {
            ...
            expect(objs[0].appendInstallScript("")).toEqual("\nfg.context.currentColorScheme = new FG_Color_ColorScheme();");
        ...
        it("should generate DynamicallyLoading objects from urls", function () {
            ...
            expect(objs[0].appendInstallScript("")).toEqual("\nfg.context.currentColorScheme = new MyCustomColorScheme();");
    ...
    describe("when loading config ", function () {
        ...
        it("should load config to config object", function (done) {
            var response = new FGravResponse();
            fg.configUrl = "test-config.json";
            fg.loadConfig(response, function () {

                try {
                    var request = jasmine.Ajax.requests.mostRecent();
                    expect(request.url).toBe("test-config.json");
                    expect(request.method).toBe('GET');

                    expect(fg.config.color.Test.uri).toEqual("test");
                    expect(fg.config.color.Test.overlays[0]).toEqual("O1");
                    expect(fg.config.color.Test.overlays[1]).toEqual("O2");
                    expect(fg.config.overlay.O1.uri).toEqual("Test_O1");
                    expect(fg.config.overlay.O2.uri).toEqual("Test_O2");
        ...
        it("should fill response when fail to load invalid config", function (done) {
            var response = new FGravResponse();
            fg.configUrl = "error.json";
            fg.loadConfig(response, function () {
                done.fail("ajax should fail");
            }, function () {
                try {
                    var request = jasmine.Ajax.requests.mostRecent();
                    expect(request.url).toBe("error.json");
                    expect(request.method).toBe('GET');

                    expect(response.errorMessage().endsWith("SyntaxError: Unexpected end of JSON input")).toEqual(true);
                    done();
                } catch (e) {
                    done(e);
                }
    ...
    describe("when loading overlay ", function () {
        ...
        it("should load dynamic overlay js file", function (done) {
            ...
            loadedO = {
                context: {
                    currentColorScheme: new FG_Color_Black()
                }
            };
